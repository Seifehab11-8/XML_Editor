...
public class JsonDecompressor {
    // Helper function to process the entire decompression
    public void processDecompression(String inputKeyFilePath, String inputCompressedFilePath, String outputFilePath) {
        if (!loadHuffmanCodes(inputKeyFilePath)) {
            System.out.println("Failed to load Huffman codes. Aborting.");
            return;
        }

        if (!loadCompressedData(inputCompressedFilePath)) {
            System.out.println("Failed to load compressed data. Aborting.");
            return;
        }

        if (!decodeData()) {
            System.out.println("Failed to decode data. Aborting.");
            return;
        }

        if (!saveDecodedData(outputFilePath)) {
            System.out.println("Failed to save decoded data. Aborting.");
        } else {
            System.out.println("Decompression completed successfully. Output saved to " + outputFilePath);
        }
    }
