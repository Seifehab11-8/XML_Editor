...
public class SecondController {
    ...
    private void initialize() {
        buttonsList = new Button[]{correction, prettify, validity, to_JSON, min, Jsondecomp,DecomXML, comp, backButton,Graph};
    ...
    private void CallJSON() throws IOException {
       XmlToJsonConverter xmlToJsonConverter =new XmlToJsonConverter();
        String jsonfile =  "E:\\JavaFxproject\\XML_Editor\\XML_Editor\\First Milestone\\demo\\src\\main\\resources\\output.json";
        xmlToJsonConverter.JSONMain(filePath,jsonfile);
        loadFileContent(jsonfile);
    }
    @FXML
    private void Call_Compression(){
        Compression compression =new Compression();
        String compressedPath = "E:\\JavaFxproject\\XML_Editor\\XML_Editor\\First Milestone\\demo\\src\\main\\resourcesXML.comp";
        compression.start_Compression(filePath);
        loadFileContent(compressedPath);
    }
    @FXML
    private void Call_JsonDecompressor()  {
        JsonDecompressor jsonDecompressor = new JsonDecompressor();
        String compressedPath = "E:\\JavaFxproject\\XML_Editor\\XML_Editor\\First Milestone\\demo\\src\\main\\resourcesXML.comp";
        String DecompressedPath = "E:\\JavaFxproject\\XML_Editor\\XML_Editor\\First Milestone\\demo\\src\\main\\resources\\JsonDecompressed.json";
        jsonDecompressor.StartJsonDecompression(compressedPath,DecompressedPath);
        loadFileContent(DecompressedPath);
    }
    @FXML
    private void Call_XmlDecompression()  {
        XMLDecompression xmlDecompression = new XMLDecompression();
        String compressedPath = "E:\\JavaFxproject\\XML_Editor\\XML_Editor\\First Milestone\\demo\\src\\main\\resourcesXML.comp";
        String DecompressedPath = "E:\\JavaFxproject\\XML_Editor\\XML_Editor\\First Milestone\\demo\\src\\main\\resources\\XMLDecompressed.xml";
        xmlDecompression.Start_XML_Decompression(compressedPath,DecompressedPath);
        loadFileContent(DecompressedPath);
    }
    ...
    @FXML private void Suggest(){String ids =GetIDS();
        if(!(ids==null||ids.isEmpty())){
    ...
    private void Call_GraphVisualizer() {
        try {
            ...
            File imageFile = new File("E:\\JavaFxproject\\XML_Editor\\XML_Editor\\First Milestone\\demo\\src\\main\\resources\\graph.jpg");
            if (!imageFile.exists()) {
